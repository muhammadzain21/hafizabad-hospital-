...
router.post("/appointments/:id/samples", allowAll, async (req, res) => {
    try {
        const { tests, guardianRelation, guardianName, cnic, phone, age, gender, address } = req.body;
...
router.get("/samples", allowAll, async (_req, res) => {
    try {
        const list = await Sample_1.default.find().sort({ createdAt: -1 }).populate('tests', 'name');
        res.json(list);
        return;
    }
    catch (err) {
