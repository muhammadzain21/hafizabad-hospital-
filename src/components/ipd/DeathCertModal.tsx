...
import DeathCertificatePage34 from '@/components/death form';
import { get as apiGet, put as apiPut } from '@/lib/api';
import BrandedHeader from '@/components/corporate/BrandedHeader';

export const DeathCertModal: React.FC<{ open: boolean; onOpenChange: (v: boolean) => void; patientId: string }>=({ open, onOpenChange, patientId })=>{
  ...
  const save = async () => {
    if (!patientId) return;
    // Minimal payload based on available patient info; extend as needed
    const payload: any = {
    ...
    await apiPut(`/api/corporate/patients/${patientId}/death`, payload);
    onOpenChange(false);
  };
